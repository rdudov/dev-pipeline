...
A bounded review packet uses `review_type=increment` and binds the submitted artifact’s version and SHA-256 digest. `increment accept` revalidates the artifact, packet, and decision; verifies that the artifact is the current submitted candidate; and emits `increment_completed` only for `approved`. Rework is performed by revising and resubmitting the same pending sequence, then obtaining a fresh digest-bound review. A completed increment cannot be reopened, and an earlier increment cannot be revised after a later one exists.
