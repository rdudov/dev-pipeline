...
3. Complete the scenario checkpoint before architecture. Stop and surface a blocking question when product semantics materially affect the implementation.
4. Complete the architecture checkpoint with the production path, owning layer, reuse plan, deletion plan, and verification plan.
5. Submit the first executable change as a walking skeleton through the real entrypoint. Map every increment to named scenarios and failure modes, declare temporary seams, and attach evidence at its honest level: `skeleton`, `integrated`, `live`, or `deployed`.
6. Build a bounded packet with `dev-pipeline context` only when a scout, scenario, architecture, diff, or live-verification question is triggered. Run exactly that packet with `dev-pipeline agent`; do not assemble a standing role pipeline.
7. Accept an increment only after focused review is approved and its required evidence gate passes. Repair findings and re-review before advancing.
8. Use `dev-pipeline owner resume` only to continue the recorded native Codex session. Use the explicit retry operation for a new attempt over existing artifacts; never label a new session as resume.
